...
from .ffn_layers import *
from .rope_positional_embedding import RopePositionEmbedding1D
...
class DinoSequenceTransformer(nn.Module):
    def __init__(
        self,
        *,
        in_chans: int = 3,
        ...
        **ignored_kwargs,
        ...
        self.rope_embed = RopePositionEmbedding1D(
            embed_dim=embed_dim,
            num_heads=num_heads,
    ...
    def prepare_tokens_with_masks(self, x: Tensor, masks=None) -> Tuple[Tensor, int]:
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        ...
        for _, blk in enumerate(self.blocks):
            if self.rope_embed is not None:
                rope_sincos = [self.rope_embed(L) for L in rope]
            else:
    ...
    def _get_intermediate_layers_not_chunked(self, x: Tensor, n: int = 1) -> List[Tensor]:
        x, L = self.prepare_tokens_with_masks(x)
        # If n is an int, take the n last blocks. If it's a list, take them
        output, total_block_len = [], len(self.blocks)
        blocks_to_take = range(total_block_len - n, total_block_len) if isinstance(n, int) else n
        for i, blk in enumerate(self.blocks):
            if self.rope_embed is not None:
                rope_sincos = self.rope_embed(L=L)
            else:
                rope_sincos = None
            x = blk(x, rope_sincos)
...
def roformer_small(patch_size=16, **kwargs):
    model = DinoSequenceTransformer(
...
def roformer_base(patch_size=16, **kwargs):
    model = DinoSequenceTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
...
def roformer_large(patch_size=16, **kwargs):
    model = DinoSequenceTransformer(
        patch_size=patch_size,
        embed_dim=1024,
        depth=24,
...
def roformer_so400m(patch_size=16, **kwargs):
    model = DinoSequenceTransformer(
        patch_size=patch_size,
        embed_dim=1152,
        depth=27,
...
def roformer_huge2(patch_size=16, **kwargs):
    model = DinoSequenceTransformer(
        patch_size=patch_size,
        embed_dim=1280,
        depth=32,
...
def roformer_giant2(patch_size=16, **kwargs):
    """
...
def roformer_7b(patch_size=16, **kwargs):
    model = DinoSequenceTransformer(
        patch_size=patch_size,
        embed_dim=4096,
        depth=40,
