...
FULLVIDEO_REVIEW_ROOT="${FULLVIDEO_REVIEW_ROOT:-${AUTO_RUN_ROOT}/manual_review_cvat_phase5_fullvideo_v2}"
...
for preflight_file in \
  "${PREFLIGHT_ROOT}/preflight_summary.json" \
  "${PREFLIGHT_ROOT}/video_summary.csv" \
  "${PREFLIGHT_ROOT}/target_bboxes.csv" \
  "${PREFLIGHT_ROOT}/source_checksums.csv"; do
  if [[ ! -f "${preflight_file}" ]]; then
    echo "Required Step 3 preflight artifact not found: ${preflight_file}" >&2
    exit 1
  fi
done

overwrite_args=()
validation_overwrite_args=()
if [[ "${OVERWRITE}" == "1" ]]; then
  overwrite_args+=(--overwrite)
  validation_overwrite_args+=(--overwrite_transfer_manifest)
fi

cd "${REPO_ROOT}"

echo "Stage 4 Phase 5 full-video CVAT review export"
echo "  Phase 3 source : ${SOURCE_ANNOTATED_ROOT}"
echo "  correction base: ${AUTO_ANNOTATED_ROOT}"
echo "  review root    : ${FULLVIDEO_REVIEW_ROOT}"
...
"${PYTHON}" \
  ...
  --output_root "${FULLVIDEO_REVIEW_ROOT}" \
...
"${PYTHON}" \
  ...
  --review_root "${FULLVIDEO_REVIEW_ROOT}" \
...
echo
echo "Stage 4 Phase 5 full-video export complete."
echo "  review index : ${FULLVIDEO_REVIEW_ROOT}/review_index.csv"
echo "  progress     : ${FULLVIDEO_REVIEW_ROOT}/video_progress.csv"
echo "  crop decisions: ${FULLVIDEO_REVIEW_ROOT}/crop_review_decisions.csv"
echo "  video tasks  : ${FULLVIDEO_REVIEW_ROOT}/videos"
echo "  summary      : ${FULLVIDEO_REVIEW_ROOT}/partition_summary.json"
echo "  validation   : ${FULLVIDEO_REVIEW_ROOT}/package_validation_summary.json"
echo "  Mac manifest : ${FULLVIDEO_REVIEW_ROOT}/mac_transfer_manifest.csv"
